...
function goOnline(userId) {
    ...
    // Auto-offline when disconnected
...
function syncAllUsersFromFirebase() {
    if (!firebaseDb) return;
    ...
    function trySync() {
        ...
        firebaseDb.ref('users').once('value').then(function(snapshot) {
            ...
            // Merge: add Firebase users that don't exist locally
            Object.keys(firebaseUsers).forEach(function(firebaseUserId) {
                if (!localUsers.find(function(u) { return u.id === firebaseUserId; })) {
                    localUsers.push(firebaseUsers[firebaseUserId]);
                }
            });
            
            Storage.set('users', localUsers);
            console.log('Synced users from Firebase:', Object.keys(firebaseUsers).length);
            
            // Reload chats to show new users
            ...
            firebaseDb.ref('users').on('child_added', function(snap) {
                ...
                if (user) {
                    ...
                    if (!users.find(function(u) { return u.id === user.id; })) {
                        ...
                        console.log('New user synced from Firebase:', user.username);
                        if (typeof loadChats === 'function') {
                            loadChats();
                        }
                    }
